...
# Project variables
PROJECT_NAME  := Ansibila.go
PROJECT_OWNER := michaelact
DESCRIPTION   := generate documentation from Ansible role in various output formats
...
GOIMPORTS_LOCAL_ARG := -local github.com/michaelact
...
checkfmt:   ## Check formatting of all go files
	...
	@ goimports -l $(GOIMPORTS_LOCAL_ARG) main.go cmd/ internal/ && echo "OK"
...
fmt:   ## Format all go files
	@ $(MAKE) --no-print-directory log-$@
	goimports -w $(GOIMPORTS_LOCAL_ARG) main.go cmd/ internal/
...
docker:   ## Build Docker image
	@ $(MAKE) --no-print-directory log-$@
	docker build --pull --tag $(DOCKER_IMAGE):$(DOCKER_TAG) --file build/Dockerfile .
